...
class Classifier_RESNET:
    ...
    def fit(self, x_train, y_train, x_val, y_val, batch_size=64, nb_epochs=100):
        if not tf.test.is_gpu_available:
            print('error')
            exit()
        # x_val and y_val are only used to monitor the test loss and NOT for training
        # y_val includes one-hot vectors for class labels
        # y_true converts y_val back to the original integer class labels
        # making it easier to calculate metrics
    ...
    def predict(self, x_test, y_true, return_df_metrics=True):
        # y_true includes the original integer class labels
        start_time = time.time()
